...
<tr>
<td>
<p style="font-weight:bold;margin-top: 10px;">{{$reports['serial_number']}}. {{$reports['heading']}} 
@if($roleId === 1)
...
@if($team_tag['tag'] != 'NA'){{trim($team_tag['parent_tag'])}} -> {{trim($team_tag['tag'])}},@endif 
@endforeach
@endif
)
</span>
...
@endphp  
({{trim($reports['source'])}}, {{trim($publish_at)}}) 
...
@if($featured_image['featured_image']!="")
<a href="{!! Helper::getImageUrl($featured_image['featured_image']) !!}" style="color:blue">Featured Image Link {{$key+1}}</a> | 
@endif
@endforeach
