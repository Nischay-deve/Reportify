...
<body>
    ...
    <table class="tableStyle" border="1" cellspacing="0" cellpadding="2" width="100%" style="border: 1px solid black;">
        <tbody>
            ...
            <tr>
                ...
                <td style="border: 1px solid #000000; text-align: left;" width="70%" align="left">{{$moduleNameArr[$datas['chapter']['id']]}} -> {{$datas['module']['name']}}</td>
            ...
            <tr>
                <td style="border: 1px solid #000000; text-align: center;" width="10%">{{ $key+1 }}</td>
                <td style="border: 1px solid #000000; text-align: left;" width="70%" align="left">{{$datas['chapter']['name']}}</td>
    ...
    <table align="left" cellspacing="3" cellpadding="3" style="margin:10pt;width:100%;text-align:left;">
        <tbody>
            ...
            $sno=1;
            @endphp             
            @foreach($data as $key=> $dataRep)
               
                <tr>
                    <td align="left" style="text-align: left;font-weight:bold;color:red">Chapter {{$key+1}}: {{$moduleNameArr[$dataRep['chapter']['id']]}} -> {{ $dataRep['chapter']['name']}}</td>
                </tr>
                @foreach($keyNews[$dataRep['chapter']['id']] as $key=>$reports)
                    <tr>
                        <td><p style="font-weight:bold;margin-top: 10px;">{{$sno}}. {{$reports['heading']}} @if($roleId === 1) <span style="color:#88b165;">(@foreach($reports['tag'] as $key=>$tag) {{$tag['tag']}}, @endforeach @foreach($reports['team_tags'] as $key=>$team_tag) @if($team_tag['tag'] != 'NA') {{$team_tag['parent_tag']}} -> {{$team_tag['tag']}}, @endif @endforeach)</span>@endif</p>
                            @php                                                 
                            $publish_at = \Carbon\Carbon::createFromTimestamp(strtotime($reports['publish_at']))->format('d/m/Y');                                                                                                                      
                            @endphp 
                            <p>
                            ({{$reports['source']}}, {{$publish_at}}) <a href="{{$reports['link']}}" style="color:blue">News Link</a> | 
