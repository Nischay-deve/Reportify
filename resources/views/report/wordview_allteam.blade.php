        ...
        <tbody>
            @foreach($dataAllTeamReport as $key => $teamData)
                ...
                <tr>
                    ...
                    <td style="background-color: #d9e2f3; color: #000000; text-align: center;" width="20%">
                        News Count ({{ $teamData['team_count'] }})
                    </td>
                </tr>

                <!-- Chapter Rows under this team -->
                @foreach (collect($data)->where('team_id', $teamData['team_id']) as $chapterData)
                    <tr>
                        <td style="border: 1px solid #000000; text-align: center;" width="10%">
                            {{ $loop->iteration }}
                        </td>
                        <td style="border: 1px solid #000000; text-align: left; padding-left:3px; mso-table-lspace:0pt; mso-table-rspace:0pt;" width="73%">
    @if(isset($chapterData['chapter']['id']))
        {{ $moduleNameArr[$chapterData['chapter']['id']] }} -> {{ $chapterData['chapter']['name'] }}
    @endif
</td>
                        <td style="border: 1px solid #000000; text-align: center;" width="7%">
